...
export const invoicesApi = baseApi.injectEndpoints({
  endpoints: (build) => ({
    ...
    createInvoice: build.mutation<Invoice, { siteId: string; vendorId?: string; taskId?: string; customTaskName?: string; customTaskUnit?: string; customTaskUnitCost?: string; quantity: string; amount?: string; description?: string; attachmentUrl?: string; status?: 'pending' | 'approved' }>({
    ...
    approveInvoice: build.mutation<Invoice, string>({
      ...
      invalidatesTags: ['Invoice', 'Vendor', 'VendorTransaction'],
    }),
    ...
    voidInvoice: build.mutation<Invoice, { id: string; reason: string }>({
      query: ({ id, ...body }) => ({ url: `/invoices/${id}/void`, method: 'POST', body }),
      invalidatesTags: ['Invoice', 'Vendor', 'VendorTransaction'],
    }),
